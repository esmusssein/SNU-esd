...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone II" OPERATION="FLOAT2FIXED" ROUNDING="TO_NEAREST" WIDTH_DATA=32 WIDTH_EXP_INPUT=8 WIDTH_EXP_OUTPUT=8 WIDTH_INT=44 WIDTH_MAN_INPUT=23 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=64 aclr clk_en clock dataa result
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone II" PIPELINE=2 SHIFTDIR="VARIABLE" SHIFTTYPE="LOGICAL" WIDTH=86 WIDTHDIST=7 aclr clk_en clock data direction distance result
//VERSION_BEGIN 11.0 cbx_altbarrel_shift 2011:04:27:21:07:08:SJ cbx_mgl 2011:04:27:21:08:59:SJ  VERSION_END

//synthesis_resources = reg 177 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  fp_fx_conv_altbarrel_shift_i7h
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[85:0]	sbit_piper1d;
	reg	[85:0]	sbit_piper2d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	reg	sel_pipec6r1d;
	wire  [7:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 86'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[343:258];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 86'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[601:516];
	// synopsys translate_off
	initial
		sel_pipec4r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		pad_w = {64{1'b0}},
		result = sbit_w[687:602],
		sbit_w = {sbit_piper2d, smux_w[515:344], sbit_piper1d, smux_w[257:0], data},
		sel_w = {sel_pipec6r1d, sel_pipec5r1d, sel_pipec4r1d, distance[3:0]},
		smux_w = {((({86{(sel_w[6] & (~ dir_w[6]))}} & {sbit_w[537:516], pad_w[63:0]}) | ({86{(sel_w[6] & dir_w[6])}} & {pad_w[63:0], sbit_w[601:580]})) | ({86{(~ sel_w[6])}} & sbit_w[601:516])), ((({86{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[483:430], pad_w[31:0]}) | ({86{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[515:462]})) | ({86{(~ sel_w[5])}} & sbit_w[515:430])), ((({86{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[413:344], pad_w[15:0]}) | ({86{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[429:360]})) | ({86{(~ sel_w[4])}} & sbit_w[429:344])), ((({86{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[335:258], pad_w[7:0]}) | ({86{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[343:266]})) | ({86{(~ sel_w[3])}} & sbit_w[343:258])), ((({86{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[253:172], pad_w[3:0]}) | ({86{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[257:176]})) | ({86{(~ sel_w[2])}} & sbit_w[257:172])), ((({86{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[169:86], pad_w[1:0]}) | ({86{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[171:88]})) | ({86{(~ sel_w[1])}} & sbit_w[171:86])), ((({86{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[84:0], pad_w[0]}) | ({86{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[85:1]})) | ({86{(~ sel_w[0])}} & sbit_w[85:0]))};
endmodule //fp_fx_conv_altbarrel_shift_i7h

//synthesis_resources = lpm_add_sub 6 lpm_compare 4 reg 412 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  fp_fx_conv_altfp_convert_u5o
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	wire  [85:0]   wire_altbarrel_shift2_result;
	...
	wire  [85:0]  lbarrel_shift_result_w;
	...
	fp_fx_conv_altbarrel_shift_i7h   altbarrel_shift2
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	defparam
		...
		barrel_direction_invert.lpm_width = 7,
	...
	assign
		...
		below_lower_limit3_oring = {wire_power2_value_result[7]},
		below_lower_limit3_w = (((wire_power2_value_result[7] & below_lower_limit3_oring[0]) | power2_value_overflow_w) & (~ below_lower_limit3_anding[7])),
		bias_value_less_1_w = 8'b01111110,
		const_bias_value_add_width_res_w = 8'b10101010,
		...
		lbarrel_shift_w = lbarrel_shift_result_w[85:23],
...
endmodule //fp_fx_conv_altfp_convert_u5o
//VALID FILE
...
module fp_fx_conv (
	...
	fp_fx_conv_altfp_convert_u5o	fp_fx_conv_altfp_convert_u5o_component (
				.aclr (aclr),
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
